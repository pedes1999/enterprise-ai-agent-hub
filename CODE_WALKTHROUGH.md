...
| `tools.ShellQuoting` | POSIX single-quote escaping (`'...'`, embedded `'` → `'\''`), extracted once `GitCloneTool` and `WriteFileTool` both needed the identical logic. `OpenPullRequestTool` is the third user. |
...
| `tools.OpenPullRequestTool` | `doExecute()`: validates a GitHub HTTPS `repositoryUrl` + `branchName`/`title`/`testCommand` (required) /`baseBranch` (defaults `main`), resolves the `GITHUB` credential (fails fast if absent -- same pattern as `GitCloneTool` and `GIT`). `doExecute` itself only validates and builds the spec; the real work is `runPipeline()`, three sequential `sandboxClient.runCommand()` calls against the SAME handle inside one `withSandbox`: (1) re-run `testCommand` for real -- if it fails, RETURN IMMEDIATELY, no further calls happen at all; (2) `git checkout -b` / `add -A` / `commit` / `push` (same `http.extraHeader` credential pattern as `GitCloneTool`, now with `GITHUB_TOKEN`) -- if this fails, return without ever reaching step 3; (3) `curl` a `POST` to `api.github.com/repos/{owner}/{repo}/pulls` (owner/repo parsed from `repositoryUrl` in Java, JSON payload built with Jackson for correct escaping), then parse the curl response for `html_url`/`number` (or `message` on a GitHub-side error) to report back. See §5 for the equivalent trace style (this one has 3 sequential sandbox calls, not 1). |
...
| Class | Responsibility |
|---|---|
| `credential.JpaCredentialResolver` | Implements `CredentialResolver`. Looks up the tenant's active `tool_credentials` row for the given kind via `ToolCredentialService.decryptActiveValue`. `envVarNameFor` maps `"GIT" -> "GIT_TOKEN"` explicitly; everything else (including `"GITHUB"`) falls through to the default `kind + "_TOKEN"`, so `"GITHUB" -> "GITHUB_TOKEN"` needed no code change when `OpenPullRequestTool` was added. |
...
| "Credential IS resolved correctly on the Java side (confirmed via logging in `AgentPromptRunner.buildSessionSpec`), but `$GIT_TOKEN`/`$GITHUB_TOKEN` reads as empty inside the sandbox" | This is a sidecar/E2B-layer bug, not a Java one -- don't waste time changing `GitCloneTool`/`OpenPullRequestTool`'s auth mechanism, changing HOW the token is used in the shell command can't fix the token never arriving at all. Isolate it by bypassing gateway-api entirely: `POST` a sandbox directly to the sidecar (`http://localhost:8090/sandboxes`) with one known test credential key, then run a command that echoes its length. If it comes back empty even here, check `server.js`'s `Sandbox.create({...})` call against the actually-installed SDK's own type definitions (`agent-runtime/sidecar/node_modules/e2b/dist/index.d.ts`, `SandboxOpts`) -- the field is `envs`, and `envVars` is silently accepted-but-ignored (this exact bug shipped once; see the README's history note). |
...
| "open_pull_request never opens anything, model says tests failed" | Working as designed -- `OpenPullRequestTool.runPipeline()` re-runs `testCommand` for real, inside the sandbox, and returns immediately on a non-zero exit without ever reaching the git/push/curl steps (see its table entry above). Check the returned `stdout`/`stderr` in the tool's result (fed back to the model) for the actual test failure, not the tool itself. |
| "open_pull_request commits/pushes but never opens the PR" | The git step succeeded but the `curl` step's response had no `html_url` -- check the reported `message` field (GitHub's own error text, e.g. bad credentials, branch already exists, head/base the same). This is a GitHub API-side failure, not a sandbox or credential-injection problem, IF the git push itself already succeeded. A generic "Validation Failed" with no further detail (the tool only surfaces the top-level `message`, not the `errors` array) used to very often be `baseBranch` not existing -- it defaulted to `"main"` if the caller omitted it, wrong for any repo whose default branch is actually `master`. Fixed: when `baseBranch` isn't given, `OpenPullRequestTool.resolveDefaultBranch()` now asks GitHub's own `GET /repos/{owner}/{repo}` for the real `default_branch` instead of guessing (falls back to `"main"` only if that lookup itself fails). |
| "GITHUB_TOKEN not found" / GitHub API returns 401 | Check `PUT /tool-credentials` was called with `credentialKind: GITHUB` for this tenant (separate from `GIT` -- see V4's rationale for why they're different rows), and that the definition's `tool_names` includes `"open_pull_request"` so `AgentPromptRunner.buildSessionSpec()` actually resolves it into the session (a definition with `git_clone` but not `open_pull_request` never touches the `GITHUB` credential kind at all). |
| "inputParameters/repositoryUrl had no effect on the prompt" | Working as designed if that `AgentDefinition`'s `input_source_type` column is null -- `AgentPromptRunner.resolveInput()` only calls `InputSourceResolverRegistry` when it's set. Check `agent_definitions.input_source_type` for the slug in question; it's a platform-wide catalog row (no per-tenant override), same as `tool_names`. |
| "Agent definition references unknown input source type" (500 from `InputSourceResolverRegistry`) | Same class of bug as `ToolCatalog`'s unknown-tool-name error -- a typo in that `agent_definitions.input_source_type` row, or an `InputSourceResolver` bean got removed/renamed without updating the rows that reference it. |
| "MANUAL_TEXT input source requires inputParameters['text']" | `ManualTextInputResolver` throws this deliberately rather than silently resolving to an empty blob -- the caller passed `inputParameters` without a `"text"` key (or left it blank). Not a bug; check what the trigger request actually sent. |
| "This tenant already has N agent executions in progress" (429 from `POST /agents/execute`) | Working as designed -- `AgentExecutionService.enqueue()`'s per-tenant concurrency cap, `app.execution.max-concurrent-per-tenant` (default 5). Check `GET /agents/executions/{id}` for that tenant's recent rows: if several are stuck `RUNNING` far longer than expected rather than genuinely all in flight, that's a *different* bug (a stuck `AgentJobWorker` run, or a crash that never called `complete()`/`fail()`) surfacing through this error, not the cap itself misbehaving. |
| "app.credentials.local-key is still the placeholder value" at startup | Working as designed -- `LocalAesGcmCredentialEncryptor`'s constructor deliberately fails fast rather than starting with a known/insecure key. Set `CREDENTIAL_LOCAL_KEY` to a real base64-encoded 256-bit key (generator command is right next to the property in `application.yml`). `application-test.yml` carries its own separate test-only key so `mvn test` is unaffected -- only a real `mvn spring-boot:run` (or deployment) needs the env var set. |
| "Missing required input(s): ..." (400 from `POST /agents/execute`) | Working as designed -- `AgentExecutionService.validateRequiredInputs()` checks the resolved `AgentDefinition.requiredInputs` (`prompt`, `repositoryUrl`, `inputParameters:{key}`) and lists every unmet one, not just the first. Check `GET /agents/definitions` for that slug's actual requirements, and confirm the trigger request's field names match exactly (`inputParameters` keys are case-sensitive). |
| "Agent definition references unrecognized required input" (500) | A data-integrity problem with that `agent_definitions.required_inputs` row, not a caller error -- either a typo (the vocabulary is exactly `prompt`, `repositoryUrl`, or `inputParameters:{key}`, nothing else), or a future value added to the vocabulary in one place but not `AgentExecutionService.isRequirementSatisfied()`. Same posture as `ToolCatalog`'s unknown-tool-name error. |
